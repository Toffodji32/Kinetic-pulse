<template>
    <div class="min-h-screen bg-gray-50">
        ...
        <nav v-if="route.name !== 'shop-login' && !route.name?.startsWith('gym-shop-login')" class="bg-white shadow-sm sticky top-0 z-50">
            <div class="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">

                <router-link :to="gymShopPath('/shop')" class="text-2xl font-black text-indigo-600 tracking-tight">
                    Kinetic <span class="text-gray-800">Shop</span>
                </router-link>

                <div class="flex items-center gap-6">
                    <router-link :to="gymShopPath('/shop')" class="text-gray-500 hover:text-indigo-600 font-medium transition-colors">
                        Catalogue
                    </router-link>

                    <router-link v-if="authStore.isAuthenticated" :to="gymShopPath('/shop/orders')"
                        class="text-gray-500 hover:text-indigo-600 font-medium transition-colors">
                        Mes commandes
                    </router-link>

                    <!-- Panier -->
                    <router-link :to="gymShopPath('/shop/cart')" class="relative">
                    ...
                    <template v-else>
                        <router-link :to="gymShopPath('/shop/login')">
...
function gymShopPath(base) {
    return gymSlug.value ? `${base}/${gymSlug.value}` : base
}

function handleLogout() {
    authStore.logout()
    router.push(gymShopPath('/shop/login'))
}
</script>