<template>
  <div class="bg-[#faf8ff] min-h-screen p-4 sm:p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-6 sm:mb-8">
      <div>
        <h2 class="text-2xl sm:text-3xl font-black font-headline tracking-tight text-indigo-600">Produits</h2>
        <p class="text-[#464554] font-medium text-sm sm:text-base">Gestion du catalogue et des stocks</p>
      </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
      <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border-l-4 border-indigo-500">
        <p class="text-[10px] font-bold uppercase tracking-widest text-[#464554] mb-1">Total produits</p>
        <p class="text-xl sm:text-3xl font-black text-[#131b2e]">{{ productStore.products.length }}</p>
      </div>
      <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border-l-4 border-green-500">
        <p class="text-[10px] font-bold uppercase tracking-widest text-[#464554] mb-1">En stock</p>
        <p class="text-xl sm:text-3xl font-black text-green-600">{{ productStore.inStock.length }}</p>
      </div>
      <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border-l-4 border-red-400">
        <p class="text-[10px] font-bold uppercase tracking-widest text-[#464554] mb-1">Rupture</p>
        <p class="text-xl sm:text-3xl font-black text-red-500">{{ productStore.outOfStock.length }}</p>
      </div>
      <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border-l-4 border-amber-400">
        <p class="text-[10px] font-bold uppercase tracking-widest text-[#464554] mb-1">Valeur stock</p>
        <p class="text-xl sm:text-3xl font-black text-amber-600">{{ formatCurrency(stockValue) }}</p>
      </div>
    </div>

    <!-- Filtres -->
    <div class="bg-white rounded-2xl p-3 sm:p-4 shadow-sm mb-6 flex flex-wrap gap-3 items-center">
      <el-input v-model="search" placeholder="Rechercher un produit..." size="large" clearable style="max-width: 100%; width: 100%; sm:max-width: 300px; sm:width: auto;">
        <template #prefix><span class="material-symbols-outlined text-lg text-[#464554]">search</span></template>
      </el-input>
      <el-select v-model="filterCategory" placeholder="Toutes catégories" size="large" clearable style="width: 100%; sm:width: 200px;" class="sm:!w-[200px]">
        <el-option v-for="cat in categories" :key="cat" :label="cat" :value="cat" />
      </el-select>
      <el-select v-model="filterStock" placeholder="Tous les stocks" size="large" clearable style="width: 100%; sm:width: 180px;" class="sm:!w-[180px]">
        <el-option label="En stock" value="in_stock" />
        <el-option label="Rupture" value="out_of_stock" />
      </el-select>
      <div class="hidden sm:flex ml-auto gap-2">
        <el-button :type="viewMode === 'grid' ? 'primary' : 'default'" @click="viewMode = 'grid'" style="border-radius: 8px;">
          <span class="material-symbols-outlined">grid_view</span>
        </el-button>
        <el-button :type="viewMode === 'table' ? 'primary' : 'default'" @click="viewMode = 'table'" style="border-radius: 8px;">
    ...
    <!-- Loading skeleton -->
    <div v-if="productStore.loading" class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 mb-6">
      <div v-for="i in 8" :key="i" class="bg-white rounded-2xl shadow-sm overflow-hidden animate-pulse">
        <div class="h-40 sm:h-48 bg-gray-100"></div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 gap-3 mb-6 sm:hidden">
        <div v-for="product in paginatedProducts" :key="product.id"
          ...
          <div class="h-28 bg-[#f2f3ff] flex items-center justify-center relative overflow-hidden">
            <img v-if="product.image" :src="mediaUrl(product.image)" class="w-full h-full object-cover" alt="" />
            <span v-else class="material-symbols-outlined text-4xl text-indigo-200">shopping_bag</span>
            <div class="absolute top-2 right-2">
              <el-tag :type="product.stock_status === 'in_stock' ? 'success' : 'danger'" size="small" round>
                {{ product.stock_status === 'in_stock' ? `${product.quantity} en stock` : 'Rupture' }}
              </el-tag>
            </div>
          </div>
          <div class="p-3">
            <h3 class="font-bold text-[#131b2e] text-xs leading-tight truncate">{{ product.name }}</h3>
            <span class="font-black text-indigo-600 text-sm"> {{ formatCurrency(product.price) }}</span>
            <div class="flex gap-2 mt-2">
              <button @click="openDetail(product)" class="flex-1 flex items-center justify-center gap-1 bg-indigo-50 text-indigo-600 text-xs font-bold rounded-lg py-2 min-h-[36px] min-w-[44px]">
                <span class="material-symbols-outlined text-sm">visibility</span>
              </button>
              <button @click="openEditModal(product)" class="flex-1 flex items-center justify-center gap-1 bg-amber-50 text-amber-600 text-xs font-bold rounded-lg py-2 min-h-[36px] min-w-[44px]">
                <span class="material-symbols-outlined text-sm">edit</span>
              </button>
              <button @click="handleDelete(product)" class="flex-1 flex items-center justify-center gap-1 bg-red-50 text-red-500 text-xs font-bold rounded-lg py-2 min-h-[36px] min-w-[44px]">
        ...
        <!-- Empty state -->
        <div v-if="paginatedProducts.length === 0" class="col-span-2 flex flex-col items-center py-12">
          <span class="material-symbols-outlined text-5xl text-gray-200 mb-3">inventory_2</span>
          <p class="text-[#464554] font-medium mb-3">Aucun produit trouvé</p>
        </div>
      </div>
      ...
      <div class="hidden sm:block">
        <!-- VUE GRILLE -->
        <div v-if="viewMode === 'grid'" class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-6">
          <div v-for="product in paginatedProducts" :key="product.id"
            ...
            <div class="h-48 bg-[#f2f3ff] flex items-center justify-center relative overflow-hidden">
              <img v-if="product.image" :src="mediaUrl(product.image)" class="w-full h-full object-cover" alt="" />
              <span v-else class="material-symbols-outlined text-6xl text-indigo-200">shopping_bag</span>
            ...
            <div class="p-4">
              <div class="flex justify-between items-start mb-1">
                ...
                <span class="font-black text-indigo-600 text-sm ml-2 flex-shrink-0">{{ formatCurrency(product.price) }}</span>
              </div>
        ...
        <!-- VUE TABLEAU -->
        <div v-else class="bg-white rounded-2xl shadow-sm overflow-hidden mb-6">
          <el-table :data="paginatedProducts" style="width: 100%" empty-text="Aucun produit trouvé">
            <el-table-column label="Produit" min-width="240">
              <template #default="{ row }">
                <div class="flex items-center gap-3 py-2">
                  <div class="w-12 h-12 rounded-xl bg-[#f2f3ff] flex items-center justify-center overflow-hidden flex-shrink-0">
                    <img v-if="row.image" :src="mediaUrl(row.image)" class="w-full h-full object-cover" />
                    <span v-else class="material-symbols-outlined text-indigo-300">shopping_bag</span>
                  </div>
                  <div>
            ...
            <el-table-column label="Stock" min-width="140">
              <template #default="{ row }">
                <el-tag :type="row.stock_status === 'in_stock' ? 'success' : 'danger'" size="small" round>{{ row.quantity }} unités</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="Statut" min-width="120" align="center">
              <template #default="{ row }">
                <el-tag :type="row.stock_status === 'in_stock' ? 'success' : 'danger'" size="small" round>{{ row.stock_status === 'in_stock' ? 'En stock' : 'Rupture' }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="Actions" min-width="180" align="center">
              <template #default="{ row }">
                <div class="flex gap-2 justify-center">
                  <el-button size="small" @click="openDetail(row)" style="background-color: #e0e7ff; color: #4f46e5; border: none; border-radius: 8px;">
                    <span class="material-symbols-outlined text-sm">visibility</span>
                  </el-button>
                  <el-button size="small" type="warning" plain @click="openEditModal(row)" style="border-radius: 8px;">
                    <span class="material-symbols-outlined text-sm">edit</span>
                  </el-button>
                  <el-button size="small" type="danger" plain @click="handleDelete(row)" style="border-radius: 8px;">
                    <span class="material-symbols-outlined text-sm">delete</span>
                  </el-button>
                </div>
              </template>
    ...
    <!-- ═══════════════════════════════════════
         MODAL CRÉATION PRODUIT (responsive)
    ═══════════════════════════════════════ -->
    <div class="sm:hidden">
      <el-drawer v-model="showCreateModal" direction="btt" size="90%" :close-on-click-modal="false"
        :append-to-body="true" :destroy-on-close="true" @closed="resetCreateForm" title="Nouveau produit">
        <template #default>
          <el-form ref="createFormRef" :model="createForm" :rules="productRules" label-position="top" size="large" class="px-2">
            <el-form-item label="Nom du produit" prop="name">
              <el-input v-model="createForm.name" placeholder="Whey Protéine 1kg" style="border-radius: 10px;" />
            </el-form-item>
            <el-form-item label="Description">
              <el-input v-model="createForm.description" type="textarea" :rows="3" placeholder="Description du produit..." style="border-radius: 10px;" />
            </el-form-item>
            <div class="grid grid-cols-2 gap-3">
              <el-form-item label="Prix (FCFA)" prop="price">
                <el-input-number v-model="createForm.price" :min="0" :precision="2" :step="0.5" style="width: 100%" />
              </el-form-item>
            ...
            <el-form-item label="Catégorie">
              <el-select v-model="createForm.category" placeholder="Choisir..." style="width: 100%">
                <el-option v-for="cat in categoryStore.categories" :key="cat.id" :label="cat.name" :value="cat.name" />
              </el-select>
            </el-form-item>
            <el-form-item label="Image du produit">
              <el-upload class="w-full" action="#" :auto-upload="false" :on-change="(f) => createForm.image = f.raw" :show-file-list="false" accept="image/*">
                <div class="w-full h-28 border-2 border-dashed border-indigo-200 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-indigo-400 transition-colors">
                  <span class="material-symbols-outlined text-3xl text-indigo-300 mb-1">upload</span>
                  <p class="text-xs text-[#464554] text-center px-2">{{ createForm.image ? createForm.image.name : 'Cliquer pour uploader' }}</p>
                </div>
              </el-upload>
            </el-form-item>
          </el-form>
    ...
    <div class="hidden sm:block">
      <el-dialog v-model="showCreateModal" title="Nouveau produit" width="560px" :close-on-click-modal="false"
        ...
        <el-form ref="createFormRef" :model="createForm" :rules="productRules" label-position="top" size="large">
          ...
          <el-form-item label="Image du produit">
            <el-upload class="w-full" action="#" :auto-upload="false" :on-change="(f) => createForm.image = f.raw" :show-file-list="false" accept="image/*">
              <div class="w-full h-32 border-2 border-dashed border-indigo-200 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-indigo-400 transition-colors">
                <span class="material-symbols-outlined text-3xl text-indigo-300 mb-2">upload</span>
                <p class="text-sm text-[#464554]">{{ createForm.image ? createForm.image.name : 'Cliquer pour uploader une image' }}</p>
              </div>
            </el-upload>
          </el-form-item>
        </el-form>
    ...
    <!-- ═══════════════════════════════════════
         MODAL MODIFICATION PRODUIT (responsive)
    ═══════════════════════════════════════ -->
    <div class="sm:hidden">
      <el-drawer v-model="showEditModal" direction="btt" size="90%" :close-on-click-modal="false"
        :append-to-body="true" :destroy-on-close="true" @closed="resetEditForm" title="Modifier le produit">
        <template #default>
          <el-form ref="editFormRef" :model="editForm" :rules="productRules" label-position="top" size="large" class="px-2">
            <el-form-item label="Nom du produit" prop="name">
            ...
            <div class="grid grid-cols-2 gap-3">
              <el-form-item label="Prix (FCFA)" prop="price">
                <el-input-number v-model="editForm.price" :min="0" :precision="2" :step="0.5" style="width: 100%" />
              </el-form-item>
              <el-form-item label="Quantité" prop="quantity">
            ...
            <el-form-item label="Changer l'image (optionnel)">
              <el-upload class="w-full" action="#" :auto-upload="false" :on-change="(f) => editForm.image = f.raw" :show-file-list="false" accept="image/*">
    ...
    ═══════════════════════════════════════ -->
    <el-drawer v-model="showDetail" direction="rtl" size="460px" :with-header="false" :append-to-body="true" class="!w-full sm:!w-[460px]">
      <div v-if="selectedProduct" class="p-4 sm:p-6 h-full overflow-y-auto">
        <div class="h-44 sm:h-56 bg-[#f2f3ff] rounded-2xl flex items-center justify-center overflow-hidden mb-6">
          <img v-if="selectedProduct.image" :src="mediaUrl(selectedProduct.image)" class="w-full h-full object-cover rounded-2xl" />
          <span v-else class="material-symbols-outlined text-7xl sm:text-8xl text-indigo-200">shopping_bag</span>
        </div>
        <div class="flex justify-between items-start mb-6">
          <div class="min-w-0 flex-1">
            <h2 class="text-xl sm:text-2xl font-black text-[#131b2e] truncate">{{ selectedProduct.name }}</h2>
            <p class="text-[#464554] text-xs sm:text-sm mt-1">{{ selectedProduct.description || 'Aucune description' }}</p>
          </div>
          <span class="text-lg sm:text-2xl font-black text-indigo-600 ml-4 flex-shrink-0">{{ formatCurrency(selectedProduct.price) }}</span>
        </div>
        <div class="space-y-3 mb-8">
          <div class="bg-[#f2f3ff] rounded-xl p-4 flex justify-between items-center">
            ...
            <div class="flex items-center gap-2">
              <span class="font-black text-lg sm:text-xl text-[#131b2e]">{{ selectedProduct.quantity }}</span>
              <el-tag :type="selectedProduct.stock_status === 'in_stock' ? 'success' : 'danger'" size="small" round>{{ selectedProduct.stock_status === 'in_stock' ? 'En stock' : 'Rupture' }}</el-tag>
            </div>
          </div>
        </div>
        <div class="bg-white border border-indigo-100 rounded-xl p-4 mb-6 shadow-sm">
          <p class="text-xs font-bold uppercase tracking-widest text-[#464554] mb-3">Ajustement stock rapide</p>
          <div class="flex items-center gap-3">
            <el-input-number v-model="stockAdjust" :min="1" :max="999" size="large" style="width: 120px; sm:width: 140px;" />
            <el-button type="success" size="large" style="border-radius: 10px; flex: 1; min-height: 44px;" @click="handleAddStock">
              <span class="material-symbols-outlined text-lg mr-1">add</span>
              Ajouter
            </el-button>
            <el-button type="danger" plain size="large" style="border-radius: 10px; flex: 1; min-height: 44px;" @click="handleDecreaseStock">
              <span class="material-symbols-outlined text-lg mr-1">remove</span>
              Retirer
            </el-button>
          </div>
        </div>
        <div class="flex gap-3">
          <el-button type="primary" size="large"
            style="flex: 1; background-color: #4f46e5; border-color: #4f46e5; border-radius: 12px; font-weight: 700; min-height: 44px;"
            @click="openEditModal(selectedProduct)">
            <span class="material-symbols-outlined text-lg mr-1">edit</span>
            Modifier
          </el-button>
          <el-button type="danger" plain size="large" style="flex: 1; border-radius: 12px; font-weight: 700; min-height: 44px;"
